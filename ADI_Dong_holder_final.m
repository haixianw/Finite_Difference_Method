function [put,Sf]=ADI_Dong_holder_final(S0,L0,tau,K,k_TC,deltat,beta,rho1,rho2,rho3,sigmaS,sigmaL,alpha,theta,r,N_S,N_L,N_T,const);
...
u = zeros(N_S,N_L);
% % optimal exercise boundary at L=L0;
 Sf = zeros(N_L,N_T);
 eps_star = K*1e-5;
 Sf(:,1)=K;
...
% Choice 2: f(L)=sqrt(L)
 theta_new(j)=theta+k_TC*((j-1)*dL)^0.4005*const;  
% Choice 3: f(L)=L^(1/3)
...
%% step 2
    ...
    A((N_S-2)*(N_L-2)) = A((N_S-2)*(N_L-2))-eta*dt*(sigmaL^2/dL...
        +alpha*(theta_new(N_L-1)-(N_L-2)*dL))/(2*dL);
    ...
    for s = 1 : N_S
     ...
     for d= 2 : N_L-1
    % last for put and first for call
            Sf(d,n+1) = S(find(abs(u(:,d)-K+S(:))< eps_star, 1, 'last'));
     end
